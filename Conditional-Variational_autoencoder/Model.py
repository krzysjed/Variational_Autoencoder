...
def image_plot(recon, img):
    #save_image(recon, 'PLOT/img' + '.png')
    #save_image(img, 'PLOT/rec' + '.png')
    ...
    plt.title("Input_standard")
    plt.axis(False)
    plt.imshow(img)

    plt.subplot(3, 2, 2)
    plt.title("Reconstruct_standard")
    plt.axis(False)
    plt.imshow(recon)
    ...
    plt.subplot(3, 2, 4)
    _ = plt.hist(recon.ravel(), bins=256, color='orange')
    _ = plt.xlabel('Intensity Value')
    _ = plt.ylabel('Count')
    _ = plt.legend(['Total'])
    ...
    plt.subplot(3, 2, 6)
    _ = plt.hist(recon[:, :, 0].ravel(), bins=256, color='Red', alpha=0.5)
    _ = plt.hist(recon[:, :, 1].ravel(), bins=256, color='Green', alpha=0.5)
    _ = plt.hist(recon[:, :, 2].ravel(), bins=256, color='Blue', alpha=0.5)
    _ = plt.xlabel('Intensity Value')
    _ = plt.ylabel('Count')
    _ = plt.legend(['Total', 'Red_Channel', 'Green_Channel', 'Blue_Channel'])
...
class CVAE(LightningModule):
    def __init__(self, num_attr=3, beta=1, max_beta=1):
        ...
        self.embed_class = nn.Linear(self.num_attr, 128 * 128)
        self.embed_data = nn.Conv2d(3, 3, kernel_size=1)

        self.encoder = nn.Sequential(
            nn.Conv2d(4, 32, kernel_size=(3, 3), stride=(1, 1), padding=1),
            nn.ReLU(),
            ResidualBlock(32),
        ...
        self.fc2 = nn.Linear(self.z_dim + self.num_attr, 128 * 4 * 4)
    ...
    def step(self, x):
        ...
        embedded_class = embedded_class.view(-1, 128, 128).unsqueeze(1)
        embedded_input = self.embed_data(x)
        x_edit = torch.cat([embedded_input, embedded_class], dim=1)

        x_hat, kl = self._run_step(x_edit, y)
